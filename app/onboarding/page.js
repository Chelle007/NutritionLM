...
function GetToKnowYouPageContent() {
  ...
  const finishOnboarding = async () => {
    const payload = {
      ...
      // Note: nutrition_goals will be generated by /api/optimal-nutrition based on user preferences
      // selectedPlan (lifestyle plan) is not stored but may influence the AI-generated goals
    };

    try {
      // First, save user preferences (without nutrition_goals)
      await insertUserPreference(payload);
      
      // Then, call the optimal-nutrition API to generate nutrition goals based on user preferences
      const nutritionResponse = await fetch('/api/optimal-nutrition', {
        method: 'POST',
        headers: {
      ...
      if (!nutritionResponse.ok) {
        ...
        console.error('Failed to generate nutrition goals:', errorText);
        // Continue anyway - user can set goals later via /api/nutrition-goals
      } else {
        const nutritionData = await nutritionResponse.json();
        console.log('✅ Nutrition goals generated:', nutritionData);
        
        // Extract the nutrition_goals jsonb object from the response
        // The response structure is: { nutritionGoals: [{ nutrition_goals: {...}, ... }] }
        const nutritionGoalsArray = nutritionData.nutritionGoals;
        if (nutritionGoalsArray && nutritionGoalsArray.length > 0) {
          const nutritionGoalsJsonb = nutritionGoalsArray[0].nutrition_goals;
          
          // Save/update nutrition goals using /api/nutrition-goals endpoint
          const saveResponse = await fetch('/api/nutrition-goals', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              nutrition_goals: nutritionGoalsJsonb
            }),
          });

          if (!saveResponse.ok) {
            const errorText = await saveResponse.text();
            console.error('Failed to save nutrition goals:', errorText);
          } else {
            const saveData = await saveResponse.json();
            console.log('✅ Nutrition goals saved to Supabase (jsonb):', saveData);
          }
        }
      }
    } catch (error) {
      console.error("Failed to complete onboarding:", error);
      // Continue anyway - user can set goals later
    }
